...
import { useVehicleEventsCard } from '@/Composables/Vehicle/Show/useVehicleEventsCard';
import { vehicleEventTypeShortLabel } from '@/Utils/labels/vehicleEventEnumLabels';
import VehicleEventFormModal from './VehicleEventFormModal.vue';
...
<template>
    <Card>
            ...
            <div class="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="mt-0.5 text-xs text-slate-500">
                        {{ props.vehicleEvents.length }} période{{
                            props.vehicleEvents.length > 1 ? 's' : ''
                        }}
                        enregistrée{{
                            props.vehicleEvents.length > 1 ? 's' : ''
                        }}
        ...
        <p
            v-if="props.vehicleEvents.length === 0"
        ...
        <ul v-else class="flex flex-col divide-y divide-slate-100 max-h-[11em] lg:max-h-[20em] xl:max-h-[11em] overflow-auto">
            <li
                v-for="item in props.vehicleEvents"
                :key="item.id"
                ...
                <div class="flex flex-col gap-1 min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                        <Badge :tone="item.hasFiscalImpact ? 'rose' : 'slate'">
                            {{ vehicleEventTypeShortLabel[item.type] }}
                        </Badge>
