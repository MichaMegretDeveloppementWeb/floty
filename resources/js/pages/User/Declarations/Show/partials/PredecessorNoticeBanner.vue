...
<template>
    <div class="flex items-start gap-3 rounded-sm border border-slate-200 border-l-2 border-l-slate-400 bg-white p-3">
        ...
        <div class="flex flex-col gap-1">
            <p class="text-sm font-medium text-slate-900">
                ...
                <template v-else> {{ predecessor.internalLabel }}</template>
            </p>
            <Link
                :href="showDeclarationRoute.url({ declaration: predecessor.id })"
                class="inline-flex w-fit cursor-pointer items-center gap-1 text-xs font-medium text-slate-600 underline-offset-2 hover:text-slate-900 hover:underline"
            >
