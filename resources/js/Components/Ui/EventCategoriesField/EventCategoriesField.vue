 ...
 * of free-text inputs (autocomplete via a native datalist of suggestions +
 * already-used categories, but any typed text is kept). Live dedup vs the locked
 * defaults and other entries. The model is the CUSTOM categories only · the
 * backend prepends the defaults.
 */
...
let uid = 0;
const datalistId = `event-categories-${(uid += 1)}-${Math.round(props.max)}`;
...
const availableSuggestions = computed<string[]>(() => {
    const used = new Set(
        [...props.lockedDefaults, ...props.modelValue]
            .map(normalizeCategory)
            .filter((key) => key !== ''),
    );

    return props.suggestions.filter((s) => !used.has(normalizeCategory(s)));
});
...
<template>
    <div class="flex flex-col gap-2">
        ...
        <p class="text-xs text-slate-500">
            Texte libre, l'auto-complétion propose les catégories existantes. Jusqu'à {{ max }} au total.
        </p>
        ...
        <div class="flex flex-col gap-2">
            ...
            >
                <div class="flex items-center gap-2">
                    ...
                    <button
                        type="button"
                        class="inline-flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-md text-slate-500 transition-colors duration-[120ms] ease-out hover:bg-rose-100 hover:text-rose-700"
