...
/**
 * Tests Feature du VehicleEventDocumentController (P1).
 *
 ...
 */
final class VehicleEventDocumentControllerTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function store_redirige_si_lutilisateur_nest_pas_connecte(): void
    {
        ...
        $response = $this->post("/app/vehicle-events/{$vehicleEvent->id}/documents", [
            'file' => $file,
        ]);
    ...
    public function store_cree_un_document_quand_lutilisateur_est_connecte(): void
    {
        ...
        $response = $this->actingAs($user)->postJson(
            "/app/vehicle-events/{$vehicleEvent->id}/documents",
            ['file' => $file],
        );

        $response->assertCreated();
        $response->assertJsonStructure(['document' => ['id', 'filename', 'downloadUrl', 'isImage']]);
        $this->assertDatabaseCount('vehicle_event_documents', 1);
    }
    ...
    public function store_rejette_un_fichier_au_mime_invalide(): void
    {
        ...
        $this->assertDatabaseCount('vehicle_event_documents', 0);
    }
    ...
    public function show_renvoie_404_quand_le_document_napparttient_pas_a_lindispo_de_lurl(): void
    {
        ...
        $unavailabilityA = VehicleEvent::factory()->for($vehicle)->create();
        $unavailabilityB = VehicleEvent::factory()->for($vehicle)->create();

        $document = VehicleEventDocument::factory()
            ->forVehicleEvent($unavailabilityB)
        ...
        $response = $this->actingAs($user)
            ->getJson("/app/vehicle-events/{$unavailabilityA->id}/documents/{$document->id}");
    ...
    public function show_stream_le_fichier_avec_le_filename_original(): void
    {
        ...
        $storagePath = "unavailability-documents/{$vehicleEvent->id}/abc.pdf";
        ...
        $response = $this->actingAs($user)
            ->get("/app/vehicle-events/{$vehicleEvent->id}/documents/{$document->id}");
    ...
    public function destroy_supprime_le_document_et_renvoie_204(): void
    {
        ...
        $storagePath = "unavailability-documents/{$vehicleEvent->id}/x.pdf";
        ...
        $response = $this->actingAs($user)
            ->delete("/app/vehicle-events/{$vehicleEvent->id}/documents/{$document->id}");
    ...
    public function destroy_renvoie_404_quand_le_document_napparttient_pas_a_lindispo_de_lurl(): void
    {
        ...
        $document = VehicleEventDocument::factory()->forVehicleEvent($unavailabilityB)->create([
            'uploaded_by' => $user->id,
        ]);

        $response = $this->actingAs($user)
            ->deleteJson("/app/vehicle-events/{$unavailabilityA->id}/documents/{$document->id}");

        $response->assertNotFound();
        $this->assertDatabaseHas('vehicle_event_documents', ['id' => $document->id]);
    }
}
