 ...
 * "Contrôle réglementaire" + "Maintenance préventive", unavailability per the
...
final readonly class RecordControlExecutionAction
{
    ...
    public function execute(RecordControlExecutionData $data, int $userId): ControlExecution
    {
        return DB::transaction(function () use ($data, $userId): ControlExecution {
            ...
            // réglementaire » (its specific axis) + « Maintenance préventive »
            // (the shared upkeep family), plus the user's additions from the
            // modal. CreateVehicleEventAction recomposes (trim + dedup).
            $categories = EventCategoryList::compose(['Contrôle réglementaire', 'Maintenance préventive'], $data->categories);
