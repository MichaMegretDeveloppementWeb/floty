...
/**
 * R-2024-013 · algorithmic pollutant categorisation
 * (CIBS art. L. 421-134).
 *
 * The full cascade lives in {@see PollutantCategory::derive()} so the
 * same logic is reused at write time (Repository) and on the frontend
 * (TS mirror).
 *
 * This rule writes the resolved category onto the pipeline context
 * from the VFC canonical fields, without re-reading the stored
 * `pollutant_category` (set by the Repository following the same
 * cascade).
 */
