 ...
 * au moins une attribution existante du véhicule.
 *
 ...
 * direct hors UI ou d'un assignment créé par un autre user pendant que
 * le formulaire d'indispo était ouvert.
 *
 * Le message utilisateur liste les dates en conflit au format français
 * pour permettre une correction rapide côté UI.
 */
final class UnavailabilityOverlapsAssignmentsException extends BaseAppException
{
    /**
    ...
    public static function withConflicts(array $dates): self
    {
        ...
        return new self(
            technicalMessage: sprintf(
                'Unavailability range overlaps %d existing assignment(s): %s',
                count($dates),
                implode(', ', $dates),
            ),
            userMessage: sprintf(
                'Période en conflit avec des attributions existantes : %s. '
                .'Retirez d\'abord ces attributions ou ajustez la plage d\'indisponibilité.',
                implode(', ', $formatted),
            ),
